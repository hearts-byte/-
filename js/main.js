        ...
        if (rooms.length === 0) {
            ...
            rooms.forEach(room => {
                ...
                roomItemDiv.addEventListener('click', () => {
                    window.location.href = `index.html?roomId=${room.id}`;
                });
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        window.allUsersAndVisitorsData = await getAllUsersAndVisitors(true); // استخدم forceRefresh لضمان جلب بيانات جديدة
    } catch (error) {
    ...
    if (chatUserId) {
        // ابحث عن بيانات المستخدم (عضو أو زائر) في قاعدة البيانات
        let userData = window.allUsersAndVisitorsData.find(user => user.id === chatUserId);
        if (!userData) {
            // أعد محاولة جلب بيانات المستخدمين والزوار مرة أخرى بشكل إجباري (قد يكون الزائر جديد ولم تتم مزامنته)
    ...
    if (!chatContainer) {
        document.body.innerHTML = '<div style="text-align: center; color: red; padding-top: 50px;">خطأ: لم يتم العثور على عنصر "chat-container". تأكد من وجوده في ملف HTML (index.html).</div>';
        return;
    }
    try {
